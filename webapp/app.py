...
def create_user_app(node_url: str, wallet_name: str, wallets_dir: str) -> Flask:
    ...
    def state_path_for_node() -> str:
        parsed = urlparse(app.config["NODE_URL"])
        port = parsed.port or 5001
        return f"data/node_{port}/state.json"
    ...
    @app.get("/state")
    def state_view():
        path = state_path_for_node()
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as exc:
            return render_template("state.html", state_path=path, state_json={}, error=str(exc))
        return render_template("state.html", state_path=path, state_json=data, error="")
