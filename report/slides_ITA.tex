...
\newcommand{\fnmark}[1]{\textnormal{[#1]}}
...
Michele Magrini, Francesco Marrocco, Leo Vincenzo Petrarca \\ \quad \\ Corso di Istituzioni di Algebra e Geometria \\ Laurea Magistrale in Matematica Applicata \\ Università di Roma "La Sapienza"
}
...
\begin{itemize}
  \item Campo finito $\mathbb{F}_p$ con curva secp256k1 \fnmark{1}:
  \[
    E: y^2 \equiv x^3 + 7 \pmod p.
  \]
  \item Gruppo dei punti con generatore $G$ e ordine primo $n$ \fnmark{2}
  \item Chiave privata: $d \in \{1,\dots,n-1\}$.
  \item Chiave pubblica: $Q = dG$.
  \item Indirizzo: ultimi 20 byte di $\mathrm{Keccak256}$.
  \item Transazione firmata con ECDSA con digest SHA-256 \fnmark{3} sul payload \fnmark{4} JSON della transazione
\end{itemize}
\end{column}
...
$k^{-1} \equiv 5^{-1} \equiv 4 \pmod{19} \to s = k^{-1}(z + rd) \bmod n = 4(11 + 10\cdot 7) \bmod 19 = 1$
...
\textbf{Verifica:}\\
Calcoliamo $X = u_1G + u_2Q$ con $u_1 = zs^{-1}$ e $u_2 = rs^{-1}$

$s^{-1} \equiv 1^{-1} \equiv 1 \pmod{19}.$

Quindi:
$
u_1 = zs^{-1} \bmod n = 11\cdot 1 \bmod 19 = 11,
\qquad
u_2 = rs^{-1} \bmod n = 10\cdot 1 \bmod 19 = 10.
$

Ora costruiamo
$X = u_1G + u_2Q = 11G + 7Q = (5 , 1) + (3 , 16) = (10 , 11)$
...
Quindi abbiamo la \textbf{Firma ECDSA:} $(r,s) = (10,1)$, il quale valore $r = x_R$ coincide con $x_X$.
...
\]
\[
d = (s_1k - z_1) r^{-1} \bmod n = 13 \cdot 2 \bmod 19 = 7
\]
