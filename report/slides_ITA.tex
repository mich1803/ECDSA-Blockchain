...
\begin{frame}{Come si Firma una transazione con ECDSA?}
Sia $z$ il digest del messaggio ridotto mod $n$, otteniamo la firma come segue:
\[
R = kG, \quad r = x_R \bmod n, \quad
s = k^{-1}(z + rd) \bmod n.
\]

La verifica controlla che il punto costruito combinando messaggio e chiave pubblica ricostruisca lo stesso valore r prodotto dal firmatario.
\[
u_1 = zs^{-1},\; u_2 = rs^{-1},\; X = u_1G + u_2Q
\]
dove $X$ è il punto ricostruito in fase di verifica che,
in caso di firma valida, coincide con $R = kG$.

\vfill
\hrule
\vspace{.45em}

La sicurezza dipende in modo critico dalla segretezza e unicità del nonce crittografico ($k$). Vedremo alla fine un possibile attacco se questo nonce viene duplicato.
\end{frame}
