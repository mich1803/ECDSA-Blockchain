...
\begin{itemize}
  \item modello ad account (non UTXO),
...
Si lavora nel campo finito $\mathbb{F}_p$ sulla curva secp256k1:
\[
E: y^2 \equiv x^3 + 7 \pmod p.
\]
Sono fissati:
\begin{itemize}
  \item un punto generatore $G$,
  \item un ordine primo $n$.
\end{itemize}
...
\begin{enumerate}
  ...
  \item si applica Keccak-256,
  \item si tengono gli ultimi 20 byte.
\end{enumerate}

\subsection{ECDSA: formula da imparare bene}
Dato un messaggio con digest $z$ (mod $n$), la firma ECDSA usa un nonce crittografico $k$:
\[
...
\begin{enumerate}
  ...
  \item si costruisce il payload della tx,
...
Il miner prende tx dalla mempool e prova nonce di PoW finché l'hash del blocco rispetta la difficoltà.
Dopo il mining:
\begin{itemize}
