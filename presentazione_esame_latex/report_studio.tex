...
Si lavora nel campo finito $\mathbb{F}_p$\footnote{Un campo finito $\mathbb{F}_p$ (con $p$ primo) è l'insieme $\{0,1,\dots,p-1\}$ con somma e prodotto modulo $p$, in cui ogni elemento non nullo ha inverso moltiplicativo.} con
\[
p=\texttt{0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F},
\]
sulla curva secp256k1:
\[
E: y^2 \equiv x^3 + 7 \pmod p.
\]
Sono fissati:
\begin{itemize}
  \item un punto generatore $G$\footnote{$G$ è fissato dallo standard secp256k1 (SEC2). In coordinate non compresse: $x_G=\texttt{79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798}$ e $y_G=\texttt{483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8}$. È il punto base da cui si generano le chiavi pubbliche come multipli scalari.},
...
\[
R=kG\footnote{$R$ è il punto ellittico ottenuto moltiplicando il generatore $G$ per il nonce crittografico $k$. È un punto "temporaneo" della firma: da $R=(x_R,y_R)$ si estrae $r=x_R \bmod n$.},\quad r=x_R\footnote{$r$ è la prima componente della firma ECDSA: si ottiene prendendo la coordinata $x$ di $R$ e riducendola modulo $n$. Se $r=0$, la firma non è valida e va rigenerata con un altro nonce $k$.} \bmod n,\quad s=k^{-1}(z+rd)\bmod n\footnote{$s$ è la seconda componente della firma ECDSA: combina digest $z$, chiave privata $d$, valore $r$ e inverso di $k$ modulo $n$. Se $s=0$, anche in questo caso la firma va rigenerata.}.
\]
...
\paragraph{Cosa fa davvero il miner matematicamente?}
In questo schema PoW il miner \textbf{non} risolve un sistema lineare/non lineare difficile.
Fa invece una ricerca per tentativi (\emph{brute force}):
\begin{enumerate}
  \item fissa i dati del blocco (header),
  \item varia il nonce $\nu$,
  \item calcola l'hash $h=H(\text{header},\nu)$,
  \item controlla se $h$ soddisfa il vincolo di difficulty.
\end{enumerate}
Quindi il ``lavoro'' è probabilistico: ogni tentativo è economico, ma servono molti tentativi
prima di trovare un hash valido.

\paragraph{La difficulty è solo da toy blockchain o anche reale?}
La difficulty non è un artificio solo didattico: è un concetto \textbf{standard} dei sistemi PoW reali.
Anche blockchain reali (storicamente Bitcoin, Ethereum pre-Merge, ecc.) usano un target di difficoltà
che regola quanto è raro trovare un hash valido.
Nel vostro progetto la versione è semplificata (prefisso di zeri esadecimali), ma il principio è lo stesso:
aumentare la difficulty rende più raro il successo per tentativo e quindi aumenta il lavoro medio richiesto.
