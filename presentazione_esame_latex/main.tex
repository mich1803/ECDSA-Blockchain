...
\begin{frame}{Difficulty: cosa succede se la cambiamo?}
\begin{itemize}
  \item Se aumentiamo difficulty ($d=3,4,\dots$):
  \begin{itemize}
    \item mining più lento (più tentativi),
    \item più tempo medio tra blocchi,
    \item demo più realistica ma meno rapida in aula.
  \end{itemize}
  \item Se diminuiamo difficulty ($d=1$):
  \begin{itemize}
    \item blocchi trovati molto velocemente,
    \item esperimento rapido ma meno significativo lato PoW.
  \end{itemize}
  \item Con default $d=2$ otteniamo un compromesso didattico: blocchi abbastanza veloci ma PoW ancora evidente.
\end{itemize}
\end{frame}
